...
void Vulkan::createInstance(VulkanTools::InstanceExtenstions extensionsRequested)
{
	...
	for (auto i = 0; i < surfaceExtensions.size(); ++i)
...
void Vulkan::createSurface(VulkanWindow* window)
{
	...
	createInfo.hwnd = reinterpret_cast<HWND>(window->winId()); //获取窗口句柄
	createInfo.hinstance = GetModuleHandle(nullptr);
	if (vkCreateWin32SurfaceKHR(m_vulkanResources->instance, &createInfo, nullptr, &surface) != VK_SUCCESS) {
...
void Vulkan::createSwapchain()
{
	...
	swapchainCreateInfo.minImageCount = surfaceCapabilities.minImageCount + 1; //设置三倍缓存
	for (auto& presentMode : presentModes)
	{
		...
		if (presentMode == vk::PresentModeKHR::eMailbox)
		{
			...
			swapchainCreateInfo.minImageCount = 3; //设置三倍缓存
...
//	vk::DeviceMemory depthImageMemory;
	m_vulkanResources->depthImage = CreateImage(vk::ImageType::e2D, vk::Format::eD32Sfloat,
	...
	m_vulkanResources->swapchain.SetDepthImage(VulkanTools::ImageResources(m_vulkanResources->depthImage.image, m_vulkanResources->depthImage.imageView, m_vulkanResources->depthImage.memory));

	m_vulkanResources->multiSampleImage = CreateImage(vk::ImageType::e2D, formatToUse,
		vk::Extent3D(swapchainCreateInfo.imageExtent, 1), vk::ImageUsageFlagBits::eColorAttachment, vk::ImageAspectFlagBits::eColor,
		vk::ImageCreateFlagBits(0), m_msaaSamples, vk::MemoryPropertyFlagBits::eDeviceLocal);
	m_vulkanResources->swapchain.SetMultiSampleImage(VulkanTools::ImageResources(m_vulkanResources->multiSampleImage.image, m_vulkanResources->multiSampleImage.imageView, m_vulkanResources->multiSampleImage.memory));
...
void Vulkan::createDebugging()
{
	...
	debugUtilsMessengerCI.pfnUserCallback = debugUtilsMessengerCallback;//debugCallback回调函数
...
{
	spdlog::info("Create2DTextureArray");
